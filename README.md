...
>Написать программу, содержащую две программы и запускать их в двух терминалах. В одной программе будет запись данных из локальной памяти в разделяемую память, а в другой программе будет запись данных из разделяемой памяти в локальную память.В программах необходимо использовать только функции ```sem_wait()```
