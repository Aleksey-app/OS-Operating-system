...
#include <fcntl.h>                                          // для O_NONBLOCK и ему подобных флагов
using std::string;
using std::cout;
using std::endl;
struct stream{
...
int main() {
    ...
    if (pipe(request) != -1);                               // создание блокируемого неименовонного канала; 
        else{
            perror("pipe[-]");}
    if (fcntl(request[1], F_GETFL, O_NONBLOCK) != -1);      // установка флагов для дискриптора записи
        else{perror("fcntl write [-]");}
    if (fcntl(request[0], F_GETFL, O_NONBLOCK) != -1);      // установка флагов для дискриптора чтения
        else{perror("fcntl read [-]");}
